    ...
    it('should find a puzzle for which the hint provider throws', async () => {
      for (let index = 1; index <= 9999; index++) {
        for (const size of [4, 5, 6, 7, 8, 9]) {
          for (const difficulty of [PuzzleDifficulty.Trivial, PuzzleDifficulty.Easy, PuzzleDifficulty.Tricky, PuzzleDifficulty.Extreme]) {
            ...
            try {
              ...
              console.log(
                `Found matching scenario on board ${PuzzleDifficulty[info.difficulty]} / ${info.boardSize}x${info.boardSize} / ${info.id}:`
              );
